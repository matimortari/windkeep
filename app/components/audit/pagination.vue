<template>
  <nav v-if="pagination && pagination.totalPages > 0" class="navigation-group w-full flex-1 justify-between" aria-label="Pagination">
    <button class="btn-info" :disabled="!pagination.hasPrev" aria-label="Previous Page" @click="auditStore.prevPage(activeOrg!.id)">
    ...
    <button class="btn-info" :disabled="!pagination.hasNext" aria-label="Next Page" @click="auditStore.nextPage(activeOrg!.id)">
