...
export default defineEventHandler(async (event) => {
  ...
  const newOwnerMembership = await db.orgMembership.findUnique({
    where: { userId_orgId: { userId: newOwnerId, orgId } },
    include: {
      user: {
        select: {
          id: true,
  ...
  const currentOwner = await db.user.findUnique({
    where: { id: user.id },
  ...
  await deleteCached(CacheKeys.userData(user.id), CacheKeys.userData(newOwnerId))
